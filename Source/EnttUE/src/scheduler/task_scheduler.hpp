...
namespace tc
{
	...
	protected:
		...
		virtual TArray<TArray<task_handle>> sort_intern(
			TMap<uint32_t, edge> &edges,
			TArray<vertex> &vertices,
			TArray<task_handle> &tasks)
		{
			// sort by incoming edge count
			auto vertex_sort = [](const vertex &task_1, const vertex &task_2) {
				if (task_1.incoming.Num() < task_2.incoming.Num())
				{
					return true;
				}
				else if (task_1.incoming.Num() == task_2.incoming.Num())
				{
					return task_1.outgoing.Num() > task_2.outgoing.Num();
				}
				return false;
			};
			...
			while (vert_stack.Num())
			{
				TArray<int32_t> group;
				{
					...
					} while (vert_stack.Num() > 0 && vert_stack.HeapTop()->incoming.Num() == 0);
				}
				...
				for (int32_t id : group)
				{
					...
					{
						vertex &outgoing_vert = vertices[outgoing];
						outgoing_vert.incoming.Remove(vert.task_id);
					}
				}
	...
	private:
		...
		std::tuple<TMap<uint32_t, edge>, TArray<vertex>, TArray<task_handle>> flatten_graph()
		{
			TMap<uint32_t, edge> edges;
			TArray<vertex> vertices;
			TArray<task_handle> tasks;
			...
			for (TSharedPtr<graph_t> &graph : graphs)
			{
				...
				while (task.IsValid())
				{
					auto index = tasks.Emplace(task_handle{++task_index, task});
					vertex &vert = vertices.Emplace_GetRef(vertex{index});

					const task_dependency &task_dep = task->get_dependencies();

					for (const int32_t &comp : task_dep.get_reads())
					{
						edge &component = edges.FindOrAdd(comp);
						component.read.Add(index);
					}

					for (const int32_t &comp : task_dep.get_writes())
					{
						edge &component = edges.FindOrAdd(comp);
			...
			for (TPair<uint32_t, edge> &kv_pair : edges)
			{
				...
				for (int32_t v_id : e.read)
				{
					...
					if (e.write.Num() == 0)
					{
						...
						TSharedPtr<task_t> &task = handle.task;
						FName name{task->get_owner()->get_name()};
