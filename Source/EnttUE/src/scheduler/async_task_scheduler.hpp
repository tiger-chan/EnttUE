...
namespace tc
{
	...
	protected:
			...
			for (int32_t i = 0; i < groups.Num(); ++i)
			{
				...
				current_tasks.Emplace(async_task_handle{
					...
					Async(
						...
						run_task(handle.task, std::forward<task_args_t &&>(args)...),
						complete_task(std::forward<task_args_t &&>(args)...))});
			}
		}

		auto run_task(TSharedPtr<task_t> task, task_args_t &&... args)
		{
			return [task, passed_args = std::forward_as_tuple(args...)]() mutable {
				return std::apply([task](auto &&... args) {
					(*task)(args...);
				},
		...
		{
			return [this, passed_args = std::forward_as_tuple(args...)]() mutable {
