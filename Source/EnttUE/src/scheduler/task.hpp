...
namespace tc
{
enum class access_t { read = 1, write = 2, read_write = read | write };

namespace details
{
template <typename... T> struct read_dependency {
};

template <typename... T> struct write_dependency {
};

template <typename... T> struct read_write_dependency {
};
} // namespace details
...
class task_dependency {
	...
	const data_t &get_reads() const
	{
		return reads;
	}
	const data_t &get_writes() const
	{
		return writes;
	}
...
template <typename... task_args_t> class task_graph;

template <typename... task_args_t> class task {
	...
	{
		work(std::forward<task_args_t>(args)...);
	}

	TSharedPtr<task<task_args_t...>> get_next()
	{
		return next;
	}

	const task_dependency &get_dependencies() const
	{
		return dependencies;
	}
	TSharedPtr<graph_t> get_owner() const
	{
		return owner.Pin();
	}
...
template <typename... task_args_t> class task_graph {
	...
	task_graph(FName name_, uint32 priority_)
	...
	inline task_ptr_t get_front()
	{
		return front;
	}

	inline FName get_name() const
	{
		return name;
	}
